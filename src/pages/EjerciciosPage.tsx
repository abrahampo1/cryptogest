...
export function EjerciciosPage({ onHelp }: { onHelp?: () => void }) {
  const { t } = useTranslation(['ejercicios', 'common'])
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        alert(result?.error ? translateError(result.error) : t('common:error'))
      }
    } catch (err) {
    ...
    <div className="space-y-4">
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge
                            ...
                            className="text-[10px]"
                          >
                            {ej.estado === "abierto" ? t('open') : t('closed')}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-1">
                            <Button
                              ...
                              title={ej.estado === "abierto" ? t('closeExercise') : t('reopenExercise')}
                              onClick={(e) => {
                                e.stopPropagation()
                                setToggleTarget(ej)
                              }}
                ...
                <Card>
                  <CardContent className="p-3">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[10px] text-muted-foreground uppercase tracking-wider">{t('billed')}</span>
                    </div>
                    <p className="text-lg font-bold">{formatCurrency(selectedStats.totalFacturado)}</p>
                    <p className="text-[10px] text-muted-foreground">
                      {t('invoiceCount', { count: selectedStats.totalFacturas })}
                    </p>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="p-3">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[10px] text-muted-foreground uppercase tracking-wider">{t('expenses')}</span>
                    </div>
                    <p className="text-lg font-bold">{formatCurrency(selectedStats.totalGastos)}</p>
                    <p className="text-[10px] text-muted-foreground">
                      {t('expenseCount', { count: selectedStats.numGastos })}
                    </p>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="p-3">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[10px] text-muted-foreground uppercase tracking-wider">{t('result')}</span>
                    </div>
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {selectedStats.resultado >= 0 ? t('profit') : t('loss')}
                    </p>
                  </CardContent>
                </Card>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-muted-foreground">{t('pendingOverdue')}</span>
                    ...
                    <div className="border-t pt-2 flex items-center justify-between text-xs">
                      <span className="text-muted-foreground">{t('totalBilled')}</span>
                  ...
                  <CardContent className="space-y-2">
                    ...
                    {selectedStats.totalAsientos > 0 && (
                      <div className="border-t pt-2">
                        <p className="text-[10px] text-muted-foreground mb-1">{t('byType')}</p>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      <p className="text-[10px] text-muted-foreground">{t('expenseCount2')}</p>
                      <p className="text-sm font-medium">{selectedStats.numGastos}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-muted-foreground">{t('totalExpenses')}</p>
                      <p className="text-sm font-medium">{formatCurrency(selectedStats.totalGastos)}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-muted-foreground">{t('averageExpense')}</p>
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent className="sm:max-w-[360px]">
          <DialogHeader>
            <DialogTitle className="text-sm">{t('newExerciseDialog')}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <div className="space-y-1.5">
              <label className="text-xs font-medium">{t('yearLabel')}</label>
            ...
            <div className="rounded-md bg-muted p-2 space-y-1">
              <p className="text-[10px] text-muted-foreground">{t('willCreate')}</p>
              ...
              <div className="flex justify-between text-xs">
                ...
                <Badge variant="default" className="text-[10px]">{t('open')}</Badge>
              </div>
            </div>
            {error && (
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="text-sm">
              {toggleTarget?.estado === "abierto"
                ? t('closeExerciseTitle', { year: toggleTarget?.anio })
                : t('reopenExerciseTitle', { year: toggleTarget?.anio })}
            </AlertDialogTitle>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction className="text-xs" onClick={handleToggleEstado}>
              {toggleTarget?.estado === "abierto" ? t('closeAction') : t('reopenAction')}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="text-sm">{t('deleteExerciseTitle', { year: deleteTarget?.anio })}</AlertDialogTitle>
            <AlertDialogDescription className="text-xs">
              {t('deleteConfirm')}
            </AlertDialogDescription>
          </AlertDialogHeader>
